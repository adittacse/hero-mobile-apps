...
const TrendingApps = ({ dataPromise }) => {
    ...
    return (
        <div className="text-center px-4 md:px-10 lg:px-20 mb-20">
            <h3 className="text-5xl text-[#001931]">Trending Apps</h3>
            ...
            <div className="grid grid-cols-4 gap-4 mt-10 mb-10">
                <Suspense fallback={<p>Loading...</p>}>
                    {
                        firstEightData.map(app => <AppCard key={app.id} app={app}></AppCard>)
                    }
                </Suspense>
            </div>
            <Link target="_blank" to="/apps" className="btn font-semibold text-white bg-[linear-gradient(90deg,#632EE3_0%,#9F62F2_100%)] rounded-sm px-[39px] py-[15px]">
                Show All
            </Link>
        </div>
    );
